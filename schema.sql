...
create table stops(stop_id integer, stop_code integer, stop_lat text, stop_lon text, zone_id integer, primary key (stop_id));
create table trips(route_id integer, service_id integer, trip_id integer, direction_id integer, block_id integer, shape_id integer, primary key (trip_id));
